...
describe('MovementsService', () => {
  ...
  it('should accept when movements correctly match balance deltas', () => {
    expect(() =>
      service.validate({
        movements: [
          movement(1, '2024-01-15', 'Salary', 3000),
          movement(2, '2024-01-20', 'Rent', -1000),
          movement(3, '2024-02-10', 'Freelance', 500),
        ],
        balances: [
          balance('2024-01-01', 1000),
          balance('2024-01-31', 3000),
          balance('2024-02-28', 3500),
        ],
      }),
    ).not.toThrow();
  });

  it('should reject when movements sum does not match the balance delta', () => {
    expect(() =>
      service.validate({
        movements: [movement(1, '2024-01-15', 'Salary', 3000)],
        balances: [
          balance('2024-01-01', 1000),
          balance('2024-01-31', 3000),
        ],
  ...
  it('should include correct period, deltas and difference in the reason', () => {
    expect.assertions(6);
    try {
      service.validate({
        movements: [movement(1, '2024-01-15', 'Salary', 3000)],
  ...
  it('should identify suspected duplicates within an invalid period', () => {
    ...
    try {
      service.validate({
        movements: [
          movement(1, '2024-01-15', 'Salary', 3000),
          movement(2, '2024-01-15', 'Salary', 3000),
        ],
        balances: [
          balance('2024-01-01', 0),
          balance('2024-01-31', 3000),
        ],
      });
    } catch (error) {
  ...
  it('should only report periods with anomalies when multiple periods exist', () => {
    ...
    try {
      service.validate({
        movements: [
          movement(1, '2024-01-15', 'Salary', 1000),
          movement(2, '2024-02-10', 'Salary', 1000),
          movement(3, '2024-02-20', 'Salary', 1000),
        ],
        balances: [
          balance('2024-01-01', 0),
          balance('2024-01-31', 1000),
          balance('2024-02-28', 2000),
        ],
      });
    } catch (error) {
      const reasons = getReasonsFromError(error);
      expect(reasons).toHaveLength(1);
      expect(reasons[0].period.end).toBe('2024-02-28');
    }
  });
  ...
  it('should handle floating point amounts without precision errors', () => {
    expect(() =>
      service.validate({
        movements: [
          movement(1, '2024-01-10', 'Coffee', -0.1),
          movement(2, '2024-01-11', 'Coffee', -0.2),
        ],
        balances: [
          balance('2024-01-01', 1),
          balance('2024-01-31', 0.7),
        ],
      }),
    ).not.toThrow();
  });
  ...
  it('should ignore movements outside all checkpoint periods', () => {
    expect(() =>
      service.validate({
        movements: [
          movement(1, '2023-12-31', 'Before first checkpoint', 9999),
          movement(2, '2024-01-15', 'In period', 500),
          movement(3, '2024-03-01', 'After last checkpoint', 9999),
        ],
        balances: [
          balance('2024-01-01', 0),
          balance('2024-01-31', 500),
        ],
      }),
    ).not.toThrow();
  });
